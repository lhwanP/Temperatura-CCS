// Programa Pronto da Temperatura do Laboratorio com média entre
// as leituras dos sensores - 24/09/2020
...
const int sensorTemp[] = {1, 2, 3, 4,5};
...
void leituradesensores()
{
  ...
  for (int j = 0; j < 5; j++)
  {
    ...
    int aux=0; // variavel auxiliar para ser o somatorio da media
    // Para evitar as grandes variações de leitura do componente LM35 são feitas 6 leitura e prevalece a média
    for (int i = 1; i <= 6; i++) // Lendo o valor do sensor de temperatura
    {
     aux += analogRead(sensorTemp[j]);
     delay(1000); // Aguarda 1 segundo
    }

    valorTemp = aux/6;   // média das 6 leituras
    ...
    lcd.write(B11011111); // Símbolo de graus 
    lcd.print("C");
    lcd.setCursor(0, 1);
  }
}
